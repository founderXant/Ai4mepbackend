from app.serializer import BuildingAreaSerializer,UserInputsSerializer
...
class CSAAUnitScheduler(APIView):
    def post(self, request, *args, **kwargs):
        # Accept the PDF file from the frontend
        ...
        try:
            # Step 2: Process the PDF using Unit Scheduler logic
            document = PdfReader(fs.path(pdf_path))
            pgs = len(document.pages)
            foundtable = []

            # Define the scrapePDF function
            def scrapePDF(startword, endword):
                for pagenumber in range(pgs):
                    ...
                    if indexstart != -1 and indexend != -1:
                        result = data[indexstart + len(startword):indexend]
                        foundtable.append(result)
                        break

            # Call scrapePDF for specific data points
            scrapePDF("Unit Overview - ", "Application")
            foundtable.append(None)  # Blank for user to input space served manually
            scrapePDF("Airflow", "Total Static Pressure")
            scrapePDF("Quantity", "Discharge")
            scrapePDF("Pressure", "in H2O")
            scrapePDF("Class ", "Fan Quantity")
            scrapePDF("Speed", "rpm")
            scrapePDF("fan motor(s)", "Warranty")

            # Process the last entry in foundtable
            last_entry = foundtable[-1]
            last_entry_no_letters = re.sub(r'[A-Za-z]', '', last_entry).strip()
            idx = last_entry_no_letters.find(".00")
            if idx != -1:
                foundtable[-1] = last_entry_no_letters[: idx + len(".00")]
                foundtable = foundtable[:-1] + foundtable[-1].split()

            # Save the data to an Excel file
            excel_path = os.path.join(fs.location, "output.xlsx")
            df = pd.DataFrame([foundtable])
            try:
                with pd.ExcelWriter(
                    excel_path, 
                    engine='openpyxl', 
                ...
                # Write the data horizontally across row 5
                    df.to_excel(writer, sheet_name='PKG AHU - HHW', index=False, header=False, startrow=5, startcol=0)
            except:
                df.to_excel(excel_path, index=False, header=False)

            # Retrn the Excel file as a response
